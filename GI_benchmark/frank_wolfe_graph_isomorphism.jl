...
include(
    "/Users/wenjiexiao/project/graph_isomorphism_from_cluster/frank_wolf_package_graph_isomorphism.jl",
)
...
function frank_wolfe_graph_isomorphism(
    ...
    while (time() - t0) < time_limit && k <= nIter
        ...
        # elseif gamma == 0.0
        # 	if num_update <= 8
        # 		println("**** Updating line search precision...")
        # 		s = step(ds) / 10
        # 		npts = Int(round(1/s)) + 1
        # 		ds = range(0.0, 1.0; length = npts)
        # 		num_update += 1
        # 	else
...
function solve_quadprog(
    ...
    if !use_package
        ...
        # Memory limit in MiB (example: 2 GiB)
        # set_optimizer_attribute(model, "limits/memory", 1024.0)
        ...
        if termination_status(model) == MOI.OPTIMAL
            ...
            println("Quadratic programming solver failed")
            return nothing
        end
    else
        x, active_set = solve_quadprog_fw_package(
            n,
            A,
            B;
            use_qua_as = use_qua_as,
